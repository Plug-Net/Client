...
public class RequestHandler implements Runnable {
    ...
    public void run() {
        ...
        PlugNetProxy.redirects.keySet().forEach(key -> {System.out.println("KEY: " + key);});
        // Parse out URL
        ...
        if(PlugNetProxy.redirects.containsKey(urlString.replace("http://", "").replace("/", "").replace("https://", ""))) {
            String old = urlString;
            urlString = PlugNetProxy.redirects.get(urlString.replace("http://", "").replace("/", ""));
            System.out.println("Redirecting to: " + urlString);
            requestString.replace(old, urlString);
        }
